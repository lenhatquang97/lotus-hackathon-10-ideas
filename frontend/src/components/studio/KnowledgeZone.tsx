  ...
  return (
    <div>
      ...
      <div className="flex items-baseline gap-3 mb-6 pb-3 border-b" style={{ borderColor: 'var(--color-fog)' }}>
        <span className="font-ui text-[10px] tracking-widest uppercase" style={{ color: 'var(--color-ash)' }}>Knowledge</span>
        <span className="font-body text-xs" style={{ color: 'var(--color-ash)' }}>Files, URLs, or text</span>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          style={{ color: 'var(--color-ink)', backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-fog)', borderRadius: '2px' }}
        ...
        <button
          ...
          style={{ backgroundColor: 'var(--color-ink)', color: 'white', borderRadius: '2px', opacity: urlInput.trim() ? 1 : 0.3 }}
        ...
        <div className="flex flex-col gap-1 mb-4">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-2 py-2 px-2 border transition-colors duration-200"
              style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-fog)', borderRadius: '2px' }}>
              <div className="flex-1 flex items-center gap-2 min-w-0">
                <span className="font-ui text-[9px] tracking-widest uppercase border px-1.5 py-0.5 shrink-0"
                  style={{ color: 'var(--color-ash)', borderColor: 'var(--color-fog)', borderRadius: '2px' }}>{item.type}</span>
                <span className="font-body text-xs whitespace-nowrap overflow-hidden text-ellipsis" style={{ color: 'var(--color-ink)' }} title={item.name}>{item.name}</span>
              </div>
              <span className={`font-ui text-[9px] tracking-wide uppercase shrink-0 ${item.status === 'ready' ? '' : 'animate-blink'}`}
                style={{ color: item.status === 'ready' ? 'var(--color-ink)' : 'var(--color-ash)' }}>{item.status}</span>
              <button className="bg-transparent border-none text-base cursor-pointer p-0 px-0.5 leading-none transition-colors duration-200"
                style={{ color: 'var(--color-fog)' }} onClick={() => onRemove(item.id)} aria-label="Remove"
                onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--color-ink)')}
                onMouseLeave={(e) => (e.currentTarget.style.color = 'var(--color-fog)')}>&times;</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div>
        <button className="bg-transparent border-none font-ui text-[10px] tracking-widest uppercase underline underline-offset-2 cursor-pointer p-0 transition-colors duration-200"
          style={{ color: 'var(--color-ash)' }} onClick={() => setPasteOpen(!pasteOpen)}
          onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--color-ink)')}
        ...
        {pasteOpen && (
          <div className="mt-3 flex flex-col gap-2 animate-fade-up">
            ...
            <button className="self-end font-ui text-[10px] tracking-widest uppercase py-2 px-4 border-none cursor-pointer transition-opacity duration-200"
              style={{ backgroundColor: 'var(--color-ink)', color: 'white', borderRadius: '2px', opacity: pasteText.trim() ? 1 : 0.3 }}
