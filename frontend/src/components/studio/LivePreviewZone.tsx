  ...
  return (
    <div className="flex flex-col gap-3 pt-2">
      {[100, 60, 80, 45, 70].map((w, i) => (
        <div key={i} className="h-3 animate-skeleton" style={{ width: `${w}%`, backgroundColor: 'var(--color-fog)', borderRadius: '2px', animationDelay: `${i * 80}ms` }} />
      ))}
    </div>
  );
}
  ...
  return (
    <div style={{ background: 'rgba(10,10,10,0.025)' }}>
      ...
      <div className="flex items-baseline gap-3 mb-6 pb-3 border-b" style={{ borderColor: 'var(--color-fog)' }}>
        <span className="font-ui text-[10px] tracking-widest uppercase" style={{ color: 'var(--color-ash)' }}>Preview</span>
        {preview.status === 'reading' && <span className="font-body text-xs animate-blink" style={{ color: 'var(--color-ash)' }}>Reading...</span>}
        {(preview.status === 'ready' || preview.status === 'partial') && (
          <span className="font-body text-xs" style={{ color: 'var(--color-ash)' }}>{preview.status === 'ready' ? 'Ready to generate' : 'Partial'}</span>
        )}
      </div>

      {preview.status === 'empty' && (
        <div className="flex items-center justify-center min-h-[200px] text-center">
          <p className="font-body text-sm leading-relaxed max-w-[200px]" style={{ color: 'var(--color-ash)' }}>
