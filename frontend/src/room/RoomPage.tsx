...
import { ContextIntroOverlay } from "./components/ContextIntroOverlay";
...
export function RoomPage({ topicId: topicIdProp }: RoomPageProps) {
    ...
    const showNameplate =
        roomState === "AGENT_SPEAKING" || roomState === "LEARNER_SPEAKING";
    const nameplateName =
        roomState === "LEARNER_SPEAKING" ? "You" : (activeAgent?.name ?? "");
    const nameplateRole =
        roomState === "LEARNER_SPEAKING" ? "" : (activeAgent?.role ?? "");

    // ─── Audio playback for agent speech (session mode) ─────────────────────────
    // Uses feedAgentAudio which both plays through speakers AND records into the mix bus
    const playAudioBytes = useCallback(
        async (bytes: ArrayBuffer) => {
            await feedAgentAudio(bytes);
        },
        [feedAgentAudio],
    );
        ...
        (event: ServerEvent) => {
            switch (event.type) {
                ...
                case "silence_warning":
                    roomEvents.onSilenceWarning(
                        useRoomStore.getState().silenceSeconds + 1,
                    );
                    break;
            }
        },
            ...
            if (isWorldMode) {
                ...
                room.connectWebSocket((entry) => {
                    ...
                    store.addTranscriptLine({
                        ...
                        speakerId:
                            entry.role === "user" ? "user" : entry.speaker,
    ...
    useEffect(() => {
        ...
        socket.onopen = () => {
            ...
            // Session start + mic capture deferred until context intro is dismissed
        };
    ...
    // ─── Start session after context intro is dismissed ─────────────────────────
    useEffect(() => {
        if (showContextIntro || isWorldMode) return;
        const socket = wsRef.current;
        if (!socket || socket.readyState !== WebSocket.OPEN) return;

        // Send start_session to backend
        socket.send(JSON.stringify({ type: "start_session" }));

        // Start mic capture + conversation recording
        (async () => {
            try {
                ...
                await startConversationRecording(stream);
                ...
                socket.send(
                    JSON.stringify({ type: "mic_active", active: true }),
                );
            } catch (e) {
                console.error("Mic access error:", e);
            }
        })();
    }, [showContextIntro, isWorldMode, startConversationRecording]);
    ...
    const handleEndSession = async () => {
        ...
        // Stop conversation recording and upload
        try {
            const audioBlob = await stopConversationRecording();
    ...
    const handleMicToggle = () => {
        ...
        if (isWorldMode) {
            ...
            wsRef.current.send(
                JSON.stringify({ type: "mic_active", active: !newMuted }),
            );
        }
    };
        ...
        <div className={`room-page${isEnding ? " ending" : ""}`}>
            ...
            {sceneError && (
                <div className="scene-loading-overlay">
                    <div className="scene-loading-content">
                        <p style={{ color: "#f97316", marginBottom: "12px" }}>
            ...
            {/* Context intro overlay — shown after scene loaded, before conversation starts */}
            {showContextIntro && !sceneLoading && !sceneError && (
                <ContextIntroOverlay
                    description={scenarioContext}
                    worldName={worldName}
                    onBegin={() => setShowContextIntro(false)}
                />
            )}
