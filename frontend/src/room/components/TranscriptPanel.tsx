...
function highlightText(text: string, regex: RegExp | null): Array<{ text: string; isVocab: boolean; word?: string }> {
  ...
  while ((match = regex.exec(text)) !== null) {
    if (match.index > lastIndex) {
      segments.push({ text: text.slice(lastIndex, match.index), isVocab: false });
    }
    segments.push({ text: match[0], isVocab: true, word: match[0].toLowerCase() });
...
function VocabWord({ text, definition, onClick }: { text: string; definition?: string; onClick?: () => void }) {
  return (
...
function VocabPopupBanner({ popups, onDismiss }: { popups: VocabPopup[]; onDismiss: (id: number) => void }) {
...
export function TranscriptPanel({ transcript, isOpen, onToggle }: TranscriptPanelProps) {
  ...
  const vocabRegex = useMemo(() => buildVocabRegex(VOCAB_WORDS), []);
  ...
  const showPopup = useCallback((word: string) => {
    const def = HARD_VOCABULARY[word];
    if (!def) return;
    const id = ++popupIdRef.current;
    setPopups(prev => [...prev, { id, word, definition: def, exiting: false }]);
                  ...
                  {segments.map((seg, i) =>
                    seg.isVocab ? (
                      <VocabWord
                        key={i}
                        text={seg.text}
                        definition={HARD_VOCABULARY[seg.word!]}
                        onClick={() => showPopup(seg.word!)}
                      />
                    ) : (
