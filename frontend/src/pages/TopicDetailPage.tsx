  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--color-bg)' }}>
      ...
      <div className="max-w-[800px] mx-auto px-12 py-16">
        ...
        <h1 className="font-display text-[48px] font-semibold leading-[1.1] mb-6" style={{ color: 'var(--color-text-primary)' }}>
          {topic.title}
        </h1>

        <p className="font-body text-[15px] leading-relaxed mb-10" style={{ color: 'var(--color-text-secondary)' }}>
          {topic.description}
        </p>

        {/* Scenario context */}
        <div className="p-6 border mb-10" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
          <span className="meta block mb-3" style={{ fontSize: '10px', letterSpacing: '0.12em' }}>Scenario Context</span>
          <p className="font-body text-[14px] leading-relaxed" style={{ color: 'var(--color-text-primary)' }}>
              ...
              <div key={char.id} className="flex gap-4 p-5" style={{ backgroundColor: 'var(--color-surface)' }}>
                ...
                <div className="flex-1">
                  <div className="flex items-baseline gap-3 mb-1">
                    <span className="font-body font-medium text-[16px]" style={{ color: 'var(--color-text-primary)' }}>{char.name}</span>
                    <span className="meta" style={{ fontSize: '10px' }}>{char.role}</span>
                  </div>
                  <p className="font-body text-[13px] leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>{char.persona}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 text-center">
          <Link to="/topics" className="font-body text-[13px] underline underline-offset-4" style={{ color: 'var(--color-text-secondary)' }}>
            Back to worlds
          </Link>
        </div>
      </div>
