...
function FilterButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      style={{ color: active ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', background: 'transparent', border: 'none' }}
    >
      {children}
      <span
        className="absolute bottom-0 left-3 right-3"
        style={{ height: '1.5px', backgroundColor: 'var(--color-accent)', display: 'block', transform: active ? 'scaleX(1)' : 'scaleX(0)', transformOrigin: 'left', transition: 'transform 180ms' }}
      />
    </button>
  );
}
...
function TopicCard({ topic, onJoin }: { topic: Topic; onJoin: () => void }) {
  return (
    ...
    >
      ...
      <div className="my-3.5 h-px" style={{ backgroundColor: 'var(--color-border)' }} />

      <h3 className="font-display text-[22px] font-semibold leading-[1.2] mb-2.5 transition-colors" style={{ color: 'var(--color-text-primary)' }}>
        {topic.title}
      </h3>

      <p className="font-body text-[13px] leading-relaxed mb-5 line-clamp-3 transition-colors" style={{ color: 'var(--color-text-secondary)' }}>
        {topic.description}
      </p>
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--color-bg)' }}>
      ...
      <div className="max-w-[1200px] mx-auto px-12 py-16">
        <div className="flex items-end justify-between mb-2">
          <h1 className="font-display text-[56px] italic" style={{ color: 'var(--color-text-primary)' }}>Available Worlds</h1>
          <Link
        ...
        <p className="font-body text-[15px] mb-12" style={{ color: 'var(--color-text-secondary)' }}>
        ...
        <div className="flex items-center mb-10 overflow-x-auto pb-2">
          ...
          <div className="w-px h-4 mx-4 flex-shrink-0" style={{ backgroundColor: 'var(--color-border)' }} />
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            <p className="font-display text-xl mb-2" style={{ color: 'var(--color-text-primary)' }}>No worlds match your filters</p>
            <p className="font-body text-sm" style={{ color: 'var(--color-text-secondary)' }}>Try adjusting your filters</p>
          </div>
        ) : (
